...
// Get log data
$stmt = Data::prepare('SELECT *, DATE_FORMAT(CONVERT_TZ(`LogTS`, "+00:00", "+08:00"), "%d%b%y %H:%i:%s") as `LogFormatTS` FROM `log`'.$where.' ORDER BY LogTS DESC LIMIT 0,50');
$stmt->execute();
...
foreach($logdata as $event) {
    $logtable .= "\t\t<tr>
            <td>".strtoupper($event['LogFormatTS'])."</td>
