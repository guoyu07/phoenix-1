...
$p['family_hphone'] = ((strlen($fam['family']['FamilyPhoneHome']) == 8) ? '<span class="muted">+852</span> '.substr($fam['family']['FamilyPhoneHome'], 0, 4).'-'.substr($fam['family']['FamilyPhoneHome'],4,4) : $fam['family']['FamilyPhoneHome']);
$p['family_mphone'] = ((strlen($fam['family']['FamilyPhoneMobile']) == 8) ? '<span class="muted">+852</span> '.substr($fam['family']['FamilyPhoneMobile'], 0, 4).'-'.substr($fam['family']['FamilyPhoneMobile'],4,4) : $fam['family']['FamilyPhoneMobile']);
$p['family_cts'] = date(DATETIME_FULL, strtotime($fam['family']['FamilyCTS']));
$p['family_address'] = nl2br($fam['family']['FamilyAddress']);
...
foreach($invoice as $i => $line) {
    ...
    $p['billed_line'] .= '<tr><td>'.date(DATE_SHORT, strtotime($line['EnrollCTS'])).'</td><td>'.strtoupper($line['CourseSubj']).str_pad($line['CourseID'], 3, '0', STR_PAD_LEFT).' - W'.$line['ClassWeek'].'P'.$line['ClassPeriodBegin'].' <em class="muted small">'.$line['StudentNamePreferred'].' '.$line['StudentNameLast'].'</td><td>'.Courses::getDueDate($line['ClassWeek']).'</td><td style="text-align:right;">$'.number_format($amount).'</td></tr>';
    $total += $amount;
}
...
$p['final_class'] = (($final_invoice < 0) ? "error" : (($final_invoice == 0) ? "success" : "pending"));
$p['final_invoice'] = number_format((-1)*($final_invoice));


echo UX::grabPage('staff/manage/print_invoice', $p, true);
